...
def multiprocess(datachunk, centroids):
    d_chunk_min = []
    for datapoint in datachunk:
        distances = []
        ...
        d_chunk_min.append(min_centroid_index)
    return d_chunk_min
...
def farm(wine_scaled):
    ...
    n = 20
    wine_split = np.array_split(wine_scaled, n)
    for i in range(n):
        globals()['split' + str(i)] = wine_split[i]
    
    count = 0
    while True:
        count += 1

        #for each chunk, assign cluster number to each data point in parallel. Choose closest centroid
        args = [(globals()['split' + str(i)], centroids) for i in range(n)]
        pool = multiprocessing.Pool(processes=n)
        results = pool.starmap(multiprocess, args)
        pool.close()
        pool.join()

        #for each chunk, gather cluster numbers from each process and store in clusters dictionary
        for i in range(n): 
            for j in range(len(results[i])):
                offset = 0
                for o in range(i):
                    offset += len(globals()['split' + str(o)])
                clusters.update({offset + j: results[i][j]})
